...
module Affine = struct
  ...
  let rec of_formula
    : type a k. (a, k) Formula.t -> t option =
    function
    | Const_int c ->
      Some (Const c)
    | Key (I x) ->
    ...
    | Binop (Plus, left, right) ->
      begin match of_formula left, of_formula right with
      | Some left, Some right -> add left right
      | _ -> None
      end
    | Binop (Minus, left, right) ->
      begin match of_formula left, of_formula right with
      | Some left, Some right -> sub left right
      | _ -> None
      end
    | _ ->
      None

  let mk_compare_or binop left right ~default =
    match left, right with
    ...
    | Var_plus_const (x, left), Var_plus_const (y, right)
      ...
      Formula.binop binop
        (Formula.const_int left)
        (Formula.const_int right)
    | Var_plus_const _, Var_plus_const _ -> default
end

let rec linearize formula =
  match formula with
  | Formula.Binop ((Equal | Less_than | Less_than_eq as binop), left, right) ->
    begin match Affine.of_formula left, Affine.of_formula right with
    | Some left, Some right ->
      Affine.mk_compare_or binop left right ~default:formula
    | _ -> formula
    end
  | Formula.And clauses ->
