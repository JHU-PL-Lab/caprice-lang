...
let split_on_pos (pgm : program_with_pos) (pos : Utils.Pos.Span.t)
  : program_with_pos * program_with_pos =
  let before (_, span) = Utils.Pos.Span.compare span pos < 0 in
...
let mk_pgms (pgm : program_with_pos) ~start_pos : (Utils.Pos.Span.t * program) list =
