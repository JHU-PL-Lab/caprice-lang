...
module Make (Key : Map.OrderedType) = struct
  ...
  *)
  ...
  (** Shift from the right list onto the left list while [key] greater than or
    equal to the head of the right list.
  ...
  let rec try_shift_onto_left_until ~with_ ~key left right =
    ...
    | (k_right, v) as binding :: tl_right ->
      ...
      if c = 0 then
        ...
        (* right head is smaller than the cursor; it needs to be on the left.
          Shift left until the key is found. *)
  ...
  (** Shift from the right list onto the left list while [key] is still greater
    than or equal to the head of the right list. *)
  and shift_onto_left_until ~key left right =
    try_shift_onto_left_until ~key left right
      ~with_:(fun ~key:_ l r -> l, None, r)

  (** Shift from the left list onto the right list while [key] is strictly
    smaller than the head of the left list. *)
  let rec shift_onto_right_until ~key left right =
    ...
    | (k_left, v) as binding :: tl_left ->
      ...
      if c = 0 then
        ...
        (* left head is greater than the cursor; continue shifting *)
  ...
  (**
    ...
    This is done by trying to shift from right onto left first. If the right
    is already greater than the key, then the key could possibly be in the left,
    so if shifting onto left fails, then we shift onto the right.
  *)
